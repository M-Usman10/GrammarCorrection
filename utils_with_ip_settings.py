...
def get_host_ip():
    """
    Get the host machine's IP address from the container's perspective.
    """
    try:
        # Dynamically get the default gateway IP (host IP for Docker)
        return subprocess.check_output(
            "ip route | awk '/default/ {print $3}'", shell=True
        ).decode().strip()
    except Exception as e:
        raise RuntimeError(f"Failed to determine host IP: {e}")

def start_triton_server():
    """
    Start the Triton server with environment variables from .env,
    passing them via -e KEY=VALUE to Docker, and correctly resolving the host IP.
    """
    ...
    cmd = [
        ...
        f"--add-host=host.docker.internal:{host_ip}",  # Dynamically add host IP
    ]
    ...
    try:
        ...
        time.sleep(2)  # Let the container start
